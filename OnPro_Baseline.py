"""OnPro implementation using Avalanche 0.6 with rotation-based augmentation,
OPE loss, and APF sampling — **adapted for CIFAR-100**.
Only dataset-related constants have been changed; all other logic
is exactly the same as in the CIFAR-10 script.
"""
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        if stride != 1 or in_planes != planes * BasicBlock.expansion:
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_planes, planes * BasicBlock.expansion, 1, stride=stride, bias=False),
...
class OnProResNet18(nn.Module):
    def __init__(self, num_classes: int, nf: int = 64):
        super().__init__()
        self.conv1 = nn.Conv2d(3, nf, 3, stride=1, padding=1, bias=False)
        ...
        self.classifier = nn.Linear(nf * 8, num_classes)  # output 100 classes
        self.proj_head = nn.Linear(nf * 8, 128)
    ...
    def forward(self, x, use_proj: bool = False):
        ...
        if use_proj:
            proj = F.normalize(self.proj_head(out), dim=1)
            return out, proj
...
class OnProStrategy(Replay):
    def __init__(
        ...
        *,
        ...
        self.ins_t = ins_t
        self.ope_loss = OPELoss(class_per_task=20, temperature=proto_t)

        self.oop = 16
        self.oop_base = model.classifier.out_features
...
benchmark = SplitCIFAR100(  # ← changed
    n_experiences=10,
    seed=0,
    fixed_class_order=list(range(100)),  # ← changed
...
for lr, epochs, alpha, mixup_a, mix_base, M in product(*param_grid.values()):
    ...
    model = OnProResNet18(num_classes=100)  # ← changed
